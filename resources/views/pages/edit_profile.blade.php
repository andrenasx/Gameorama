...
<section class="container g-0 mx-auto my-4 col-lg-7">
    <section class="profile-widget bg-white rounded mb-3">
        <form method="POST" action="{{ route('edit_profile', $member->username) }}" enctype="multipart/form-data" id="edit_form">
            @csrf
            {!! method_field('patch') !!}
            <row class="mb-3">
            ...
            <section class = "container w-100 mt-2 form-group p-2">
                <div class="mb-4">
                    ...
                    <input type="text" class="form-control" id="new-post-title" name = "full_name" value = "{{$member->full_name}}">
                </div>
                <div class="mb-4">
                    <label for="exampleFormControlTextarea1" class="form-label">Biography</label>
                    <textarea class="form-control" id="exampleFormControlTextarea1" name = "bio" rows="5" placeholder="Add a bio to your profile!">{{$member->bio}}</textarea>
                </div>
