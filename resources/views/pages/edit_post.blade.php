        ...
        <section class="container w-100 mt-4 form-group">
            <form method="POST" action="{{ route('update_post', ['id_post' => $post->id]) }}" enctype="multipart/form-data" autocomplete="off">
                ...
                <section id="title" class="mb-5">
                    ...
                    <input type="text" class="form-control" id="new-post-title" name="title" value="{{ old('title', $post->title) }}" required>
                ...
                <section id="body" class="mb-5">
                    ...
                    <textarea class="form-control" id="editor-body" name="body" rows="5">{{ old('body', $post->body) }}</textarea>
                ...
                <section id="topics" class="mb-5">
                    ...
                    <select id="select2-topics" class="form-control" multiple="multiple" name="topics[]" required>
                        @foreach($topics as $topic)
                            <option value="{{ $topic->name }}" @if($post->topics->containsStrict('id', $topic->id)) selected @endif >{{ $topic->name }}</option>
                        @endforeach
                    </select>
